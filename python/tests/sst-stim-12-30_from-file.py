...
# flicker frequency
stim_left_flicker_freq = 12  # in Hz
stim_right_flicker_freq = 30  # in Hz
frame_rate = 120  # refresh rate in Hz
stim_left_frames_per_cycle = int(frame_rate / stim_left_flicker_freq)
stim_right_frames_per_cycle = int(frame_rate / stim_right_flicker_freq)
...
for file in file_list:
    img = visual.ImageStim(
        ...
        size = (600, 600),
        units = 'pix')
    stim_img_right.append(img)

# make left and right stims at 12 and 30 Hz independently
while True:
    for frameN in range(frame_rate):
        # Draw left stim
        stim_img_left[frameN // stim_left_frames_per_cycle].draw()
        # Draw right stim
        stim_img_right[frameN // stim_right_frames_per_cycle].draw()
